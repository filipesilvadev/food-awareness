(function ( $ ) {
	...
	MapField.prototype = {
		...
		initDomElements: function () {
			this.$docent_pro_core = this.$container.find( '.rwmb-map-docent-pro-core' );
			this.docentprocore = this.$docent_pro_core[0];
		...
		initMapElements: function () {
			var defaultLoc = this.$docent_pro_core.data( 'default-loc' ),
				latLng;
			...
			this.map = new google.maps.Map( this.docentprocore, {
				center: latLng,
				zoom: 14,
				streetViewControl: 0,
			...
			$address.autocomplete( {
				source: function ( request, response ) {
					var options = {
						...
						'region': that.$docent_pro_core.data( 'region' )
					};
