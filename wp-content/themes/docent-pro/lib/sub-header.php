                ...
                <?php
                    ...
                    if(isset($wp_query->queried_object->name)){
                        if (get_theme_mod( 'header_title_enable', true )) {
                            if($wp_query->queried_object->name != ''){
                                if($wp_query->queried_object->name == 'product' ){
                                    echo '<h2 class="page-leading">'.esc_html__('Shop','docent-pro').'</h2>';
                                }else{
                        ...
                        if( is_search() ){
                            if (get_theme_mod( 'header_title_enable', true )) {
                                $docent_pro_text = '';
                                $docent_pro_first_char = esc_html__('Search','docent-pro');
                                if( isset($_GET['s'])){ 
                            ...
                            if (get_theme_mod( 'header_title_enable', true )) {
                                if (get_theme_mod( 'header_title_text', 'Blog' )){
                                    echo '<h2 class="page-leading">'. wp_kses_post(get_theme_mod( 'header_title_text','“ News & Blog ”' )).'</h2>';
                                }
                            }
                        } 
                        else if( is_single()){
                            ...
                            if (get_theme_mod( 'header_title_enable', true )) {
                                ...
                                if ( $docent_pro_subtext != ""){
                                    echo '<h3 class="page-leading">'. $docent_pro_subtext .'</h3>';
                                }
                            }
                        } 
                        else{
